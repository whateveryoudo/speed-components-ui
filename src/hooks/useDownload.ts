...
export const handleExceptDown = (
  ...
  if (disposition) {
    // 获取 filename*=UTF-8'' 后面的部分
    const filenameMatch = disposition?.match(/filename\*=UTF-8''([^;]+)/);
    if (filenameMatch) {
      resFileName = decodeURIComponent(filenameMatch[1]);
    } else {
      // 降级处理：如果没有 filename*=UTF-8'' 格式，尝试 filename= 格式
      const fallbackMatch = disposition?.match(/filename="([^"]+)"/);
      if (fallbackMatch) {
        resFileName = decodeURIComponent(fallbackMatch[1]);
      }
    }
  }
